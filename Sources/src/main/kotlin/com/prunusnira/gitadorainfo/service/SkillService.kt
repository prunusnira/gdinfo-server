...
class SkillService {
	...
	: Map<Int, ArrayList<SkillTable>> {
		...
		when(ptype) {
			...
			9 -> all.addAll(getSkillTop(Const.INFOVER.EXC.value,
					user.id, gtype))
			10 -> {
			...
			15 -> all.addAll(getSkillTop(Const.INFOVER.FU.value,
					user.id, gtype))
			16 -> {
				hot.addAll(getSkillTarget(Const.INFOVER.FU.value,
						user.id, "h", gtype))
				other.addAll(getSkillTarget(Const.INFOVER.FU.value,
						user.id, "o", gtype))
			}
		}
		skillMap.put(0, all)
	...
	fun loadSnapshot(uid: Int, date: String, gtype: String): String {
		val file = File(SecretConst.snapshotRealpathServer+uid+"/"+date+"_"+gtype+".json")
	//	val file = File(SecretConst.snapshotRealpathLocal+uid+"/"+date+"_"+gtype+".json")
